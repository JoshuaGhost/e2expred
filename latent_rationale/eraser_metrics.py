...
from latent_rationale.utils import (
    Annotation,
...
class PositionScoredDocument:
    ...
    def from_results(cls, instances: List[dict], annotations: List[Annotation], docs: Dict[str, List[Any]],
        ...
        """Creates a paired list of ann ids/docids/predictions/truth values"""
...
def partial_match_score(truth: List[Rationale], pred: List[Rationale], thresholds: List[float]) -> List[Dict[str, Any]]:
    ...
    Computes an instance-level (ann) micro- and macro-averaged F1 score.
    ...
    instance (ann + document) precisions and recalls, averaging those,
...
def score_hard_rationale_predictions(truth: List[Rationale], pred: List[Rationale]) -> Dict[str, Dict[str, float]]:
    """Computes instance (ann)-level micro/macro averaged F1s"""
    scores = dict()
    truth = set(truth)
    pred = set(pred)
...
def verify_instance(instance: dict, docs: Dict[str, list], thresholds: Set[float]):
    ...
    for rat in instance['rationales']:
        ...
        if docid not in docid:
            error = True
            logging.info(
                f'Error! For instance ann={instance["annotation_id"]}, docid={docid} could not be found as a preprocessed document! Gave up on additional processing.')
            continue
        ...
        for h1 in rat.get('hard_rationale_predictions', []):
            ...
            for h2 in rat.get('hard_rationale_predictions', []):
                ...
                if len(set(range(h1['start_token'], h1['end_token'])) & set(
                    ...
                    logging.info(
                        f'Error! For instance ann={instance["annotation_id"]}, docid={docid} {h1} and {h2} overlap!')
                    error = True
            if h1['start_token'] > doc_length:
                logging.info(
                    f'Error! For instance ann={instance["annotation_id"]}, docid={docid} received an impossible tokenspan: {h1} for a document of length {doc_length}')
                error = True
        ...
        if len(soft_rationale_predictions) > 0 and len(soft_rationale_predictions) != doc_length:
            logging.info(
                f'Error! For instance ann={instance["annotation_id"]}, docid={docid} expected classifications for {doc_length} tokens but have them for {len(soft_rationale_predictions)} tokens instead!')
            error = True
    ...
    for docid, count in docids.items():
        if count > 1:
            error = True
            logging.info(
                'Error! For instance ann={instance["annotation_id"]}, docid={docid} appear {count} times, may only appear once!')

    classification = instance.get('classification', '')
    if not isinstance(classification, str):
        logging.info(
            f'Error! For instance ann={instance["annotation_id"]}, classification field {classification} is not a string!')
        error = True
    classification_scores = instance.get('classification_scores', dict())
    if not isinstance(classification_scores, dict):
        logging.info(
            f'Error! For instance ann={instance["annotation_id"]}, classification_scores field {classification_scores} is not a dict!')
        error = True
    ...
    if not isinstance(comprehensiveness_classification_scores, dict):
        logging.info(
            f'Error! For instance ann={instance["annotation_id"]}, comprehensiveness_classification_scores field {comprehensiveness_classification_scores} is not a dict!')
        error = True
    ...
    if not isinstance(sufficiency_classification_scores, dict):
        logging.info(
            f'Error! For instance ann={instance["annotation_id"]}, sufficiency_classification_scores field {sufficiency_classification_scores} is not a dict!')
        error = True
    if ('classification' in instance) != ('classification_scores' in instance):
        logging.info(
            f'Error! For instance ann={instance["annotation_id"]}, when providing a classification, you must also provide classification scores!')
        error = True
    if ('comprehensiveness_classification_scores' in instance) and not ('classification' in instance):
        logging.info(
            f'Error! For instance ann={instance["annotation_id"]}, when providing a classification, you must also provide a comprehensiveness_classification_score')
        error = True
    if ('sufficiency_classification_scores' in instance) and not ('classification_scores' in instance):
        logging.info(
            f'Error! For instance ann={instance["annotation_id"]}, when providing a sufficiency_classification_score, you must also provide a classification score!')
        error = True
    ...
    for instance in instances:
        ...
        if len(has_soft_rationales) > 0:
            ...
            if len(has_soft_rationales) != len(instance['rationales']):
                error = True
                logging.info(
                    f'Error: instance {instance["ann"]} has soft rationales for some but not all reported documents!')
        if len(has_soft_sentences) > 0:
            ...
            if len(has_soft_sentences) != len(instance['rationales']):
                error = True
                logging.info(
                    f'Error: instance {instance["ann"]} has soft sentences for some but not all reported documents!')
        if 'thresholded_scores' in instance:
...
def main(args):
    ...
    and the ann, divided by the union of the two spans, is larger than
